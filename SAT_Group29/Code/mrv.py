...
def select_literal_mrv(cnf_clauses, unassigned_vars):
    """Select the variable with the fewest occurrences in the remaining clauses."""
    var_counts = {var: 0 for var in unassigned_vars}

    # count occurrences of each unassigned variable in the clauses
...
def dp_solver(cnf_clauses, assignments, stats, use_pure_literals=True):
    ...
    # Use the MRV heuristic to select the next literal
    literal = select_literal_mrv(cnf_clauses, unassigned_vars)
...
def solve_puzzles(sudoku_rules, sudoku_puzzles, output_prefix):
    ...
    print(f"Length: {len(backtracker)} Mean: {np.mean(backtracker)}, SD: {np.std(backtracker)}")
